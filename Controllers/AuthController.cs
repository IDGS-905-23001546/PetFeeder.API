...
public class TempController : ControllerBase
{
    private readonly DualWriteService _dual;
    private AppDbContext _db => _dual.Db;
    public TempController(DualWriteService dual) { _dual = dual; }
    ...
    [HttpGet("otp")]
    public async Task<IActionResult> OTP([FromQuery] string email)
    {
        var usuario = await _db.Usuarios.FirstOrDefaultAsync(u => u.Email == email);
        if (usuario == null)
            return NotFound(new { exito = false, mensaje = "Usuario no existe." });
        if (usuario.Verificado)
            return Ok(new { exito = true, mensaje = "Cuenta ya verificada.", codigo = "VERIFICADO" });
        ...
        if (otp == null)
            return NotFound(new { exito = false, mensaje = "No hay código activo." });

        return Ok(new { exito = true, codigo = otp.Codigo, expiraEn = otp.ExpiraEn });
    }
